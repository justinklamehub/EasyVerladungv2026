...
const S = {
  ...
  markBtn: (col: "B" | "V") => ({
    ...
    border: "none",
    cursor: "pointer",
    background: col === "B" ? "rgba(59,130,246,0.12)" : "rgba(101,163,13,0.12)",
    color: col === "B" ? "#2563eb" : "#65a30d",
  ...
  checkBox: (checked: boolean) => ({
    ...
    background: checked ? "rgba(217,119,6,0.1)" : "transparent",
    cursor: "pointer",
    flexShrink: 0,
  ...
  if (submitted) {
    return (
      <div style={{ ...S.page, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
        <CheckCircle2 size={64} color="#16a34a" style={{ marginBottom: 24 }} />
        <div style={{ fontSize: 22, fontWeight: 700, color: "#0f172a", marginBottom: 8 }}>
          Checkliste eingereicht
        </div>
        <div style={{ fontSize: 14, color: "#475569", textAlign: "center", marginBottom: 32, maxWidth: 320 }}>
          Die Gefahrgut-Checkliste{localKennzeichen ? <> für <strong style={{ color: "#0f172a" }}>{localKennzeichen}</strong></> : ""} wurde erfolgreich übermittelt.
        </div>
